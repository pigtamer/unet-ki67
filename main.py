...
from data_wrap import train_dataset, val_dataset, test_dataset
from tvt_behavior import train_loop, test_loop
...
valGene, n_val = 0, 0
testGene, n_test = 0, 0
file_writer = SummaryWriter(logdir + "/metrics")  # torch to tb

model = smunet(loss=loss_name)

# TODO: implement training dataset
train_sampler = DistributedSampler(train_dataset)
train_loader = torch.utils.data.DataLoader(
    train_dataset, sampler=train_sampler, batch_size=bs
)
...
if not flag_test:
    ...
    for k in range(50, 51):
        ...
        auclist = []
        legs = ["Luck"]
        show_id_list = ["1-1", "1-2", "1-3", "2-1", "2-2", "2-3", "3-1", "3-2", "3-3"]
        ...
        for id_loocv_t in range(8, 9):
            ...
            for kk, (tx, ty) in zip(range(n_test), testGene):
                if data_name != "ALL":
                    if kk < test_list[id_loocv_t][0]:
                        continue
                    if kk > test_list[id_loocv_t][1]:
                        break
                ...
                plt.imsave(
                    "/raid/ji/DATA/KimuraLIpng/ihc_%d.png" % kk,
